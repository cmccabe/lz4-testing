...
static int write_compressed_segment(LZ4_stream_t *stream,
                                    FILE *ifp, FILE *ofp)
{
    size_t buf_size, res;
    char buf[512];
    ...
    actual_case = LZ4_compress_continue(stream, buf, obuf, buf_size);
    if (actual_case == 0) {
...
static int run_test(LZ4_stream_t *stream, char *ipath, char *opath)
{
    ...
    while (1) {
      int res = write_compressed_segment(stream, ifp, ofp);
      if (res == 0) {
          ret = EXIT_SUCCESS;
          break;
      }
